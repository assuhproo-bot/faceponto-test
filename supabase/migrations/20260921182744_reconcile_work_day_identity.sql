...
begin
  ...
  if target_day.schedule_version_id <> p_schedule_version or target_day.journey_end <> p_journey_end
    ...
    -- A schedule can be corrected after a day has already been calculated.
    -- Keep the same work-day identity and create a new calculation revision
    -- with the corrected schedule instead of leaving the recalculation queued.
    update public.work_days
    set schedule_version_id = p_schedule_version,
        journey_end = p_journey_end,
  ...
  if previous.id is not null then
    ...
    if prior_entry.id is not null and prior_entry.delta_minutes <> 0 then
