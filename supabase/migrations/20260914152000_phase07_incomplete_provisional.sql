...
begin
  ...
  next_state := case when coalesce((p_result->>'provisional')::boolean,false) or p_result->>'net_balance_minutes' is null then 'provisional' else 'final' end::public.attendance_state;
  ...
  if previous.id is not null then
    ...
    if prior_entry.id is not null then
      ...
      values(p_company,p_employee,target_day.id,calculation.id,next_revision,-prior_entry.delta_minutes,
        'Estorno automÃ¡tico da revisÃ£o anterior',prior_entry.id);
    end if;
  end if;
