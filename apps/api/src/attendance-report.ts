...
const total = (report: AttendanceReport, field: 'plannedMinutes' | 'workedMinutes' | 'balanceMinutes') =>
...
export function attendanceXlsx(report: AttendanceReport): Buffer {
  const header = ['Data', 'Matricula', 'Colaborador', 'Previsto', 'Trabalhado', 'Saldo', 'Estado'];
  const detail = report.rows.map((row) => [row.date, row.registration, row.employee, minutes(row.plannedMinutes), minutes(row.workedMinutes), minutes(row.balanceMinutes), row.state]);
  const summary = ['TOTAL', '', '', minutes(total(report, 'plannedMinutes')), minutes(total(report, 'workedMinutes')), minutes(total(report, 'balanceMinutes')), ''];
  const sheetRows = [
  ...
  const sheet = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><cols><col min="1" max="1" width="14" customWidth="1"/><col min="2" max="2" width="16" customWidth="1"/><col min="3" max="3" width="30" customWidth="1"/><col min="4" max="6" width="14" customWidth="1"/><col min="7" max="7" width="16" customWidth="1"/></cols><sheetData>${sheetRows}</sheetData></worksheet>`;
...
export function attendancePdf(report: AttendanceReport): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const document = new PDFDocument({ size: 'A4', margin: 36, info: { Title: 'FacePonto - Relatorio de jornadas' } });
    const chunks: Buffer[] = [];
    document.on('data', (chunk: Buffer) => chunks.push(chunk));
    document.on('end', () => resolve(Buffer.concat(chunks)));
    document.on('error', reject);
    document.font('Helvetica-Bold').fontSize(16).text('FacePonto - Relatorio de jornadas');
    ...
    document.font('Helvetica-Bold').fontSize(9).text(`Previsto ${minutes(total(report, 'plannedMinutes'))}   Trabalhado ${minutes(total(report, 'workedMinutes'))}   Saldo ${minutes(total(report, 'balanceMinutes'))}`);
    document.moveDown().font('Courier-Bold').fontSize(7).text('DATA        MATRICULA       COLABORADOR                PREV.  TRAB.  SALDO  ESTADO');
    document.font('Courier').fontSize(7);
    for (const row of report.rows) {
      if (document.y > 780) { document.addPage(); document.font('Courier-Bold').text('DATA        MATRICULA       COLABORADOR                PREV.  TRAB.  SALDO  ESTADO'); document.font('Courier'); }
      const line = `${row.date.padEnd(11)} ${row.registration.slice(0, 14).padEnd(14)} ${row.employee.slice(0, 25).padEnd(25)} ${minutes(row.plannedMinutes).padStart(6)} ${minutes(row.workedMinutes).padStart(6)} ${minutes(row.balanceMinutes).padStart(6)}  ${row.state.slice(0, 12)}`;
      document.text(line);
    }
