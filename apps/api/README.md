...
- `POST /v1/employees`, sujeito às políticas RLS e com matrícula sequencial automática quando ela não é informada;
- `PATCH /v1/employees/:id`, edição/desativação com versão otimista e auditoria;
- `GET/POST /v1/departments` e `PATCH /v1/departments/:id`, cargos versionados para agrupamento administrativo e financeiro;
- `GET/POST /v1/company-payment-settings`, valores gerais da empresa;
- `GET/POST /v1/department-payment-settings` e `GET/POST /v1/payment-settings`, valores por cargo e por funcionário;
- `GET /v1/payment-rates`, resolução por item na ordem funcionário, cargo e empresa;
- `GET/POST /v1/payment-days`, adicionais por data: almoço, janta, diária, madrugada, sábado e serão;
...
- `GET /v1/financial-attendance`, espelho financeiro por funcionário e período, com totais de horas e valores;
- `GET/POST/PATCH/DELETE /v1/absence-categories`, categorias de ausência ativas, versionadas e auditadas;
- `GET/POST/DELETE /v1/day-justifications`, justificativa de dia inteiro, com o abono de horas gravado no próprio registro;
...
O cálculo de jornada usa os blocos da escala, e não a posição ordinal das batidas. Assim, em uma escala `08:00–12:00` e `14:00–18:00`, uma batida às 14h ocupa a entrada da tarde e as posições da manhã permanecem ausentes. Dias sem batidas só geram falta quando existe jornada prevista. As justificativas abonadas acrescentam as horas previstas ao saldo e mantêm esse resultado mesmo que a categoria seja alterada posteriormente.

Os relatórios XLSX e PDF usam a mesma consulta financeira de `GET /v1/financial-attendance`: filtros de funcionário, período e empresa produzem as mesmas horas, adicionais e totais nos três destinos.

Execute `npm.cmd run typecheck` e `npm.cmd run api:test` com o Supabase local ativo. Os testes de integração usam Auth, PostgREST e PostgreSQL reais, criam dois tenants sintéticos e limpam somente seus próprios dados ao terminar. `SUPABASE_SECRET_KEY` é usada somente pelo endpoint de provisionamento para criar a identidade individual do terminal e consumir o código de pareamento. Ela nunca deve ser enviada ao APK ou ao painel.
