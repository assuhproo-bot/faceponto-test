...
function PaymentSummary({ employee, companyId, token, rows, from, to }: { employee: Employee; companyId: string; token: string; rows: Array<{ date: string; worked: number; overtime: number }>; from: string; to: string }) {
  ...
  async function saveUnits(date: string, field: 'meal_units' | 'dinner_units' | 'daily_allowance_units' | 'night_shift_units' | 'saturday_units', units: number) { const current = dayByDate.get(date); const updated = { meal_units: current?.meal_units ?? 0, dinner_units: current?.dinner_units ?? 0, daily_allowance_units: current?.daily_allowance_units ?? 0, night_shift_units: current?.night_shift_units ?? 0, saturday_units: current?.saturday_units ?? 0, [field]: Math.max(0, Math.min(10, units || 0)) }; setPendingDate(date); setMessage(''); try { await api('/v1/payment-days', token, { method: 'POST', body: JSON.stringify({ company_id: companyId, employee_id: employee.id, local_date: date, expected_version: current?.version ?? null, ...updated }) }); await load(); } catch (cause) { setMessage(cause instanceof ApiError ? cause.message : 'Não foi possível salvar esta quantidade.'); } finally { setPendingDate(''); } }
  ...
  return <section className="payment-summary"><div><strong>Apuração de pagamento</strong><span>Total previsto: <b>{cents(total)}</b></span><small>Informe a quantidade de cada adicional pago em cada dia. Horas normais e extras vêm das batidas acima.</small></div><div className="table-wrap"><table><thead><tr><th>Data</th><th>Horas</th><th>Extra</th>{columns.map(([, label]) => <th key={label}>{label}</th>)}</tr></thead><tbody>{rows.map((row) => { const flags = dayByDate.get(row.date); return <tr key={row.date}><td>{dayLabel(row.date)}</td><td>{minutes(row.worked)}</td><td>{minutes(row.overtime)}</td>{columns.map(([field, label]) => <td key={field}><input type="number" min="0" max="10" step="1" aria-label={`${label} em ${row.date}`} value={flags?.[field] ?? 0} disabled={Boolean(pendingDate)} onChange={(event) => void saveUnits(row.date, field, Number(event.target.value))} /></td>)}</tr>; })}{!rows.length && <Empty colSpan={8} />}</tbody></table></div>{message && <p className="form-message">{message}</p>}</section>;
}
