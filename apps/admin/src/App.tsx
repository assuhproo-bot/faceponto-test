...
function Timesheet({ employee, punches, days, locations, companyId, token, selectedLocationId, from, to, onSaved }: { employee: Employee | undefined; punches: Punch[]; days: WorkDay[]; locations: Location[]; companyId: string; token: string; selectedLocationId: string; from: string; to: string; onSaved: () => void }) {
  ...
  const [rates, setRates] = useState({ regular_hour_cents: 0, overtime_hour_cents: 0 });
  const slotOrder = ['entry', 'break_start', 'break_end', 'exit'];
  useEffect(() => { let active = true; if (!employee) return; void Promise.all([api<{ data: CompanyPaymentSettings | null }>(withQuery('/v1/company-payment-settings', { company_id: companyId }), token), api<{ data: EmployeePaymentSettings | null }>(withQuery('/v1/payment-settings', { company_id: companyId, employee_id: employee.id }), token)]).then(([company, individual]) => { if (!active) return; const source = individual.data ?? company.data; setRates({ regular_hour_cents: source?.regular_hour_cents ?? 0, overtime_hour_cents: source?.overtime_hour_cents ?? 0 }); }).catch(() => { if (active) setRates({ regular_hour_cents: 0, overtime_hour_cents: 0 }); }); return () => { active = false; }; }, [companyId, employee?.id, token, ratesRefresh]);
  ...
  const totals = dailyRows.reduce((result, row) => { const money = Math.round(row.extra * rates.overtime_hour_cents / 60) - Math.round(row.shortage * rates.regular_hour_cents / 60); return { worked: result.worked + row.worked, overtime: result.overtime + row.extra, absences: result.absences + row.shortage, balance: result.balance + row.extra - row.shortage, money: result.money + money }; }, { worked: 0, overtime: 0, absences: 0, balance: 0, money: 0 });
