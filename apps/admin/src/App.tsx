...
function DailyAllowances({ employee, companyId, token, dates, from, to }: { employee: Employee; companyId: string; token: string; dates: string[]; from: string; to: string }) {
...
function Timesheet({ employee, punches, days, locations, companyId, token, selectedLocationId, from, to, onSaved }: { employee: Employee | undefined; punches: Punch[]; days: WorkDay[]; locations: Location[]; companyId: string; token: string; selectedLocationId: string; from: string; to: string; onSaved: () => void }) {
  ...
  const dailyRows = useMemo(() => {
    ...
    return [...keys].sort().map((date) => {
      ...
      return { date, slots, extras, punchCount: (punchByDay.get(date) ?? []).length, calculation: currentCalculation(dayByKey.get(date) ?? { attendance_calculations: [] } as WorkDay) };
    });
  }, [days, punches, from, to]);
  const totals = dailyRows.reduce((result, row) => { const absenceMinutes = row.punchCount === 0 && (row.calculation?.planned_minutes ?? 0) > 0 ? row.calculation!.planned_minutes : 0; return { worked: result.worked + (row.calculation?.worked_minutes ?? 0), overtime: result.overtime + (row.calculation?.gross_overtime_minutes ?? 0), balance: result.balance + (row.calculation?.net_balance_minutes ?? -absenceMinutes), absences: result.absences + absenceMinutes }; }, { worked: 0, overtime: 0, balance: 0, absences: 0 });
  ...
  return <section className="panel timesheet"><div className="timesheet-heading"><div><p className="eyebrow">APURAÇÃO E RELATÓRIO DO PERÍODO</p><h2>{employee?.name ?? 'Funcionário'}</h2><p>{employee?.registration ? `Matrícula: ${employee.registration}` : 'Matrícula não informada'} · {from || 'Início não selecionado'} até {to || 'Fim não selecionado'}</p></div><small>Clique duas vezes em um horário para corrigir. Clique duas vezes em uma célula vazia para incluir uma batida esquecida.</small></div><div className="timesheet-summary"><article><span>Horas trabalhadas</span><strong>{minutes(totals.worked)}</strong></article><article><span>Horas extras</span><strong>{minutes(totals.overtime)}</strong></article><article><span>Horas faltantes</span><strong>{minutes(totals.absences)}</strong></article><article><span>Saldo do período</span><strong>{minutes(totals.balance)}</strong></article></div><PaymentSettingsForm employee={employee} companyId={companyId} token={token} /><DailyAllowances employee={employee} companyId={companyId} token={token} from={from} to={to} dates={dailyRows.map((row) => row.date)} /><ReportDownloads companyId={companyId} employeeId={employee?.id ?? ''} from={from} to={to} token={token} /><div className="table-wrap"><table><thead><tr><th>Data</th><th>Entrada</th><th>Saída 1</th><th>Entrada 2</th><th>Saída 2</th><th>Outras</th><th>Trabalhado</th><th>Extra</th><th>Saldo</th></tr></thead><tbody>{dailyRows.map((row) => <tr key={row.date}><td>{dayLabel(row.date)}</td>{cell(row, 'entry', 'entrada')}{cell(row, 'break_start', 'saída para intervalo')}{cell(row, 'break_end', 'retorno do intervalo')}{cell(row, 'exit', 'saída')}<td>{row.extras.map((punch) => fortalezaTime(punch.timestamp)).join(' · ') || '—'}</td><td>{row.punchCount === 0 && (row.calculation?.planned_minutes ?? 0) > 0 ? 'Falta' : minutes(row.calculation?.worked_minutes)}</td><td>{minutes(row.calculation?.gross_overtime_minutes)}</td><td>{minutes(row.punchCount === 0 && (row.calculation?.planned_minutes ?? 0) > 0 ? -row.calculation!.planned_minutes : row.calculation?.net_balance_minutes)}</td></tr>)}{!dailyRows.length && <Empty colSpan={9} />}</tbody></table></div><div className="timesheet-totals"><strong>Horas trabalhadas: {minutes(totals.worked)}</strong><strong>Horas extras: {minutes(totals.overtime)}</strong><strong>Faltas: {minutes(totals.absences)}</strong><strong>Saldo: {minutes(totals.balance)}</strong></div>{message && <p className="form-message">{message}</p>}</section>;
}
